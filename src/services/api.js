...
const API_BASE_URL = 'https://base.funfinder.ge/en/api/v1';
const API_V2_BASE_URL = 'https://base.funfinder.ge/en/api/v2';
const API_V3_BASE_URL = 'https://base.funfinder.ge/ka/api/v3';
const API_V5_BASE_URL = 'https://base.funfinder.ge/en/api/v5';
...
// Create v3 axios instance for v3 endpoints
...
addAuthInterceptor(axiosInstanceV3);
...
addResponseInterceptor(axiosInstanceV3);
...
export const categoryApi = {
  ...
  // Get company feed by category ID - v1 endpoint
  getCompanyFeedByCategory: (categoryId) => apiService.get(`/category/company/feed/${categoryId}`),
};
...
export const sliderApi = {
  // Get all sliders - uses v3 feed endpoint
  getAll: () => axiosInstanceV3.get('/slider/feed'),
  
  // Get slider by ID (if available)
  getById: (id) => apiService.get(`/slider/${id}`),
  
  // Create new slider - uses multipart/form-data with image, title, description
  create: (formData) => {
    return apiService.post('/slider/create', formData, {
      headers: {
        'Content-Type': 'multipart/form-data'
      }
    });
  ...
  // Update existing slider (note: not documented in API, may not be available)
  update: (id, formData) => {
    return apiService.post(`/slider/update/${id}`, formData, {
      headers: {
        'Content-Type': 'multipart/form-data'
      }
    });
  ...
  // Delete slider
  delete: (id) => apiService.delete(`/slider/delete/${id}`),
};
